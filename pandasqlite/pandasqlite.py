...
def export(ingestion, db):
    logger.debug("START: export")
    result = []
    for i in ingestion:
        df = pd.read_sql(f"SELECT * FROM '{i['hash']}'", db)
        df = df.iloc[:, 1:]  # remove first column (index generated by database)
        df.columns = i["orig_columns"]  # [df.columns[0]] + i["orig_columns"].to_list() [if index preserved]
        result.append(df)
...
def snapshot_data(df):
    ...
    for column in df.columns:
        ...
        samples = df[column].sample(min(5, len(df)))
